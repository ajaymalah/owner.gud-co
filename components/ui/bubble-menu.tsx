...
export default function BubbleMenu() {
  ...
  // Set up scroll container and listener
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const middle = scrollTop + el.clientHeight / 2;
      ...
      sections.forEach((section) => {
        if (section.sub) {
          section.sub.forEach((sub) => {
            ...
            if (!subEl) return;
            const top = subEl.offsetTop;
            const bottom = top + subEl.offsetHeight;
      ...
      if (firstEl && lastEl) {
        ...
        const fill = Math.min(Math.max(middle - start, 0), totalHeight);
  ...
  const isParentActive = (section: any) =>
    section.sub?.some((sub: { id: string }) => sub.id === activeSection);
